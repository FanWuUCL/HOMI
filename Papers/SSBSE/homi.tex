...
\begin{figure}[ht]
	...
	\vspace{-1em}
	\caption{Pareto Fronts of HOMs and FOMs for each subject. Lower and lefthand solutions dominate high and righthand solutions.}\label{fig_pareto}
\end{figure}
...
\begin{table}[ht]
\centering
\caption{Improvement compared with the original when HOMs are combined with Deep-Parameter-optimised libraries. Each cell reports the time improvement followed by memory improvement in percentage. `T' or `M' indicates it is most time-saving or memory-saving HOM/library.}
\label{tab_deep}
\resizebox{\textwidth}{!}{
...
\multirow{3}{*}{\rotatebox[origin=c]{90}{\emph{espresso}}} & Original & 0/0            & 0.8/0.1       & 0.7/0.2      & \multirow{3}{*}{\rotatebox[origin=c]{90}{\emph{gawk}}} & Original & 0/0           & 5.4/1.6        & -0.2/2.3     \\
                          & HOM(T)   & 6.9/-0.2       & 4.8/\textbf{0.1}       & 4.7/\textbf{0.2}      &                       & HOM(T)   & 9.8/-0.1      & 5.6/\textbf{1.6}        & 5.4/\textbf{2.3}      \\
                          & HOM(M)   & 6.5/1.7        & 4.7/\textbf{1.8}       & \textbf{6.7}/\textbf{1.7}      &                       & HOM(M)   & 6.1/4.3       & \underline{4.1}/\textbf{5.8}        & 4.8/\textbf{5.5}      \\
\hline
\multirow{3}{*}{\rotatebox[origin=c]{90}{\emph{flex}}}     & Original & 0/0            & 15.7/-2.6     & -1.1/0.6     & \multirow{3}{*}{\rotatebox[origin=c]{90}{\emph{sed}}}  & Original & 0/0           & 7.9/-1208      & 5.6/2.0      \\
                          & HOM(T)   & 2.3/0          & 14.4/-2.6     & -Inf/-Inf    &                       & HOM(TM)   & 18.2/0.5      & \underline{5.8}/-1208      & \underline{4.1}/0.9      \\
                          & HOM(M)   & -10.3/19.7     & -3.5/\textbf{19.7}     & -Inf/-Inf    &                       &    &             &              &           \\
\hline
\end{tabular}
}
\end{table}

According to the table, there are 10 out of 28 cases (bold numbers) when combining HOM-improved program with Deep-Parameter-optimised library, the performance is at least the same as the best performance of the HOM or library it is combined from, and is strictly better on four cases.
However, there are three cases (underlined) that the combination makes their performance worse.
In most of the cases, the performance lies between the performance of the HOM and the library that it is combined from.
In the case of \emph{flex}, we found that the most memory-saving library even breaks the functionality of HOMs (indicated by `-Inf' in the table).
